// this code is a copy from the the link below very usful program. 
// https://github.com/Maya-Fey/CYBR372-Assignment-1
...
public class FileEncryptor {
    ...
    public static void main(String[] args) {
        // Convert the arguments to char arrays, wipe the originals, and then enter into
        // a new function (hopefully the args[] will GC, giving just an extra layer of
        // protection)
     ...
     * 
     * 0x09 (1 byte) Block size (1 byte) Key size (1 byte) <length of algorithm
     * string> (1 byte) algorithm string (length bytes long) <length of cipher
     * string> cipher (length bytes long) salt for cipher key (16 bytes) salt for
     * MAC key (16 bytes) IV (blocksize bytes long) MAC of plaintext (32 bytes)
     * 
     * The rationale for this file format is as follows:
     * 
     * - The 0x09 guard is just to make it so that more files are rejected early
     * rather than causing IO errors or decrypt errors. Not foolproof, obviously, a
     * random file has a 1/256 chance of having 0x9 on its own. - Block and key size
     * are only one byte as they themselves are in bytes. A block size of 255 bytes
     * would be over two thousand bits, which is not something we're likely to see.
     * - Separate salts for cipher key and mac key as it's a convenient way to
     * produce two different keys from the same master secret - MAC at the beginning
     * so we can read it without dropping out of the cyrpto stream - MAC is of
     * plaintext because mac-then-encrypt is about as secure as encrypt-than-mac,
     * but the code complexity of mac-then-encrypt is much smaller, only requiring
     * the file to be read once. (even though encrypt-than-mac is more efficient
     * compute-wise)
     */
    ...
    private static final void enc(InputParams params) {
        try {
            ...
            // Technically, I could have made one chunk key and split it in half
            ...
            try (FileInputStream fis = new FileInputStream(inFile)) {
                try (FileOutputStream fos = new FileOutputStream(outFile)) {
                    ...
                    // For all data in the file to encrypt, encrypt and then write to the output
                    // file
            ...
            System.out.println("The selected cipher (" + params.getCryptParams().getAlgorithm() + "/"
                    + params.getCryptParams().getCipher()
                    + ") is not available on this system. Consider upgrading your JRE.");
            System.exit(0);
    ...
    private static final void dec(InputParams params) {
        ...
        try (FileInputStream fis = new FileInputStream(inFile)) {
            ...
            // If metadata reading was successful, overwrite the file and create a new
            // zero-length one
            outFile.createNewFile();

            // Initialize the cipher stream from the remaining file data
            // For every byte we read, write it to the target file, as well as feed it into
            // the MAC
            ...
            // If the MACs aren't equal, report as decryption failure, delete the file, and
            // exit
            if (!Arrays.equals(givenMAC, computedMAC)) {
                System.out.println(
                        "Decrypted file didn't pass verification. Either your password was incorrect, or the file has been corrupted.");
                outFile.delete();
                System.exit(0);
            }
            ...
            System.out.println(
                    "The cipher to decrypt the file is not available on this system. Consider upgrading your JRE.");
            System.exit(0);
        } catch (NoSuchPaddingException e) {
            System.out.println("Internal error in the application. Likely a programming bug.");
            System.exit(0);
    ...
    /**
     * Parses input parameters from main(). Ensures that they are textually valid,
     * but performs no external validation (ie, doesn't confirm that the given input
            ...
            case "aes":
                if (useDefaultLength) {
                    ...
                    if (length != 16 && length != 24 && length != 32) {
                        System.out
                                .println("Fatal: Invalid key length specifier for AES, must be one of [128, 192, 256]");
                        System.exit(0);
                    }
            ...
            case "blowfish":
                if (useDefaultLength) {
                    ...
                    if (length < 4 || length > 56) {
                        System.out.println(
                                "Fatal: Invalid key length specifier for Blowfish, must be in the range 32-448");
                        System.exit(0);
                    }
    ...
    private static final class Util {
         ...
         * @return An array of chars, each representing one nibble/hexadecimal
         *         character. Hex will be in upper case [0-9A-F]
         */
        ...
        public static final byte[] fromHex(char[] str) {
            if ((str.length & 1) == 1)
                throw new IllegalArgumentException(
                        "A hex string must have an even number of characters. Consier adding a leading zero.");
    ...
    private static final class CryptUtil {
        ...
        // For a command line file encryption utility, fast access isn't important. High
        // iteration count will add an extra layer of security
        ...
        public static final byte[] keyFromPassword(int keyBytes, byte[] salt, char[] password)
